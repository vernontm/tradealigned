...
async function sendWelcomeEmail(email: string, planId: RayAiPlanId) {
  ...
  const heading = isTrial
    ? "You're in. 7 days of full access for $1."
    : "Welcome to Ray AI.";
  const body = isTrial
...
<p>On day 8 your card auto-renews at $29.99/mo. Cancel any time before then with one click from your billing page and you're never charged past the $1.</p>
...
<p>Jump back in whenever you're ready.</p>`;
  ...
  await sendEmail({
    ...
    html: brandedEmail({
      preheader: isTrial
        ? "7 days of full access for $1. Cancel anytime."
